	...
	foreach($infos as $info){
		?>
          <tr>
            ...
            <td width="8%" align="center"><?php if($info['passed']==1){ echo "已审核";}else{ echo "未审核";}?><br />[<?php if($info['replycontent']==""){echo "<font color='#f00'>未回复</font>";}else{echo "已回复";} ?>]
            </td>
            <td align="center" width="12%"><a href="###"
			onclick="show(<?php echo $info['id']?>, '<?php echo new_addslashes($info['name'])?>')"
			title="<?php echo "查看或回复"?>"><?php echo "查看/回复"?></a> | <a
			href='?m=guestbook&c=guestbook&a=delete&id=<?php echo $info['id']?>'
...
&nbsp;&nbsp;
<input type="submit" class="button" name="dosubmit" onClick="document.myform.action='?m=guestbook&c=guestbook&a=check_noregister'" value="移动到[未审核]"/>
&nbsp;&nbsp;
